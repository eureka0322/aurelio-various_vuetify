        ...
        <v-layout row wrap>
          <v-flex xs5>
            <span style="font-size: 35px; font-weight: 200; color: green;"><span
              style="font-size: 35px; font-weight: 600; color: green;">2. </span>{{TRANS('Listado de centros')}}</span>
          </v-flex>
          <v-flex xs5>
            <v-text-field
              v-model="search"
                ...
                <v-menu bottom right>
                  ...
                  <v-list style="background-color: black;">
                    <v-list-tile @click="editItem(props.item)">
                      ...
                      <v-list-tile-title dark flat style="color: white;">{{TRANS('Editar')}}
                      </v-list-tile-title>
                    </v-list-tile>
                    <v-list-tile avatar @click="deleteItem(props.item)">
                      <v-icon color="green">delete</v-icon>
                      <v-list-tile-title>
                        <v-list-tile-sub-title style="color: white;">{{TRANS('Eliminar')}}
...
<script>
  import json from '../Json/Centros.json'
  import EditDelete from '../Common/EditDelete.vue'
  ...
  export default {
    ...
    mounted() {
      ...
      this.tableData = json.centros_list
